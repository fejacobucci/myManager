...
module.exports = function dynamicTag(out, tag, getAttrs, renderBody, args, props, componentDef, key, customEvents) {
  if (tag) {
    ...
    if (typeof tag === "string") {
      ...
      if (renderBody) {
        out.aR_(tag, attrs, key, componentDef, addEvents(componentDef, customEvents, props));
        renderBody(out);
        out.aS_();
      } else {
    ...
    out.bf(key, component, IS_SERVER && componentDef && componentDef._E_ & FLAG_WILL_RERENDER_IN_BROWSER);
    renderBody(out);
    out.ef();
  }
};
...
function attrsToCamelCase(attrs) {
  ...
  for (var key in attrs) {
    result[changeCase.aQ_(key)] = attrs[key];
  }
